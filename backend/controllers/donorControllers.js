...
const jwt = require('jsonwebtoken');  // 👈 تمت الإضافة
...
const verifyAndSaveDonor = (req, res) => {
    ...
    db.query(query, [
        ...
        if (err) {
            ...
            return res.status(500).json({ message: "Error saving donor to database" });
        }
        pendingDonors.delete(email);
        res.status(201).json({
...
const getAllDonors = (req, res) => {
    ...
    db.query(sql, (err, result) => {
        ...
        res.status(200).json({ success: true, donors: result });
    });
};
...
const loginDonor = (req, res) => {
    ...
    db.query(sql, [email], async (err, result) => {
        ...
        // توليد token (تأكد من وجود JWT_SECRET في .env)
...
const forgotPassword = async (req, res) => {
    try {
        ...
        db.query(checkQuery, [email], async (err, result) => {
            ...
            if (!emailSent) return res.status(500).json({ message: "Failed to send reset code" });
...
const verifyResetCode = (req, res) => {
    try {
        ...
        if (!resetRequest) return res.status(404).json({ message: "No reset request found" });
